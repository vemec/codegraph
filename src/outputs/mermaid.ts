...
import { findNodes } from '../query/query.js';
 ...
 * Meant to paste into a PR or a .md — Mermaid renders well when bounded.
 */
export function toMermaid(graph: Graph, symbol: string, depth = 2): string {
  ...
  const out: Array<string> = ['flowchart LR'];

  for (const id of ids) {
    const n = byId.get(id);

    if (n) {
      out.push(`  ${safeId(idx.get(id)!)}["${label(n)}"]`);
    }
  }
  ...
  for (const e of graph.edges) {
    if (included.has(e.from) && included.has(e.to)) {
      ...
      out.push(
        `  ${safeId(idx.get(e.from)!)} ${arrow}|${e.kind}| ${safeId(idx.get(e.to)!)}`,
      );
    }
  }
